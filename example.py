...
model.add(
    ...
    tags=[
        "advanced",
        "magic",
    ],  # for certain workflow engines that make use of parameter tags
...
subparams.add(
    "param_1", type=float, default=5.5, description="Some floating point setting."
)
...
subsubparams.add(
    "param_3", type=int, default=2, description="A subsetting's subsetting"
)
...
print(
    "For debugging purposes we can output a human readable representation of Parameter objects. Here's the first one:"
)
print(params[0])
print
...
print(
    "We can create dictionaries of arguments on our own that we want to validate against the model."
...
print(
    "We can validate these arguments against the model, and get a dictionary with parameter types correctly casted "
...
print(
    "Validation enforcement levels can be 0, 1 or 2 for type-casting, restriction-checking and required argument presence. "
...
try:
    model.validate_args(
        validated_2, enforce_restrictions=2
    )  # , enforce_type=0, enforce_required=0
...
directives_1 = parse_cl_directives(
    "--input_ctd exampleTool_preset_params.ctd --write_param_ctd new_preset_params.ctd"
)
pretty_print(directives_1)
directives_2 = parse_cl_directives(
    "-inctd exampleTool_preset_params_2.ctd -toolctd ",
    input_ctd="inctd",
    write_tool_ctd="toolctd",
    prefix="-",
)
...
print(
    "Finally, writing CTDs with logging information, passing a dictionary"
